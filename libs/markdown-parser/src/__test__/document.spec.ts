...
describe('findNodeAtCursorPosition()', () => {
  it('returns a node at the given position', () => {
    const doc = new MarkdownDocument('# Hello *World*\n\n- Item 1\n- Item **2**');

    expect(doc.findNodeAtCursorPosition([1, 1])).toMatchObject({
      type: 'heading'
    });

    expect(doc.findNodeAtCursorPosition([1, 3])).toMatchObject({
      type: 'text',
      literal: 'Hello '
    });

    expect(doc.findNodeAtCursorPosition([1, 9])).toMatchObject({
      type: 'emph',
      firstChild: {
        type: 'text',
        literal: 'World'
    ...
    expect(doc.findNodeAtCursorPosition([1, 10])).toMatchObject({
      type: 'text',
      literal: 'World'
    });

    expect(doc.findNodeAtCursorPosition([3, 1])).toMatchObject({
      type: 'item'
    });

    expect(doc.findNodeAtCursorPosition([3, 3])).toMatchObject({
      type: 'text',
      literal: 'Item 1'
    });

    expect(doc.findNodeAtCursorPosition([4, 8])).toMatchObject({
      type: 'strong',
      firstChild: {
        type: 'text',
        literal: '2'
    ...
    expect(doc.findNodeAtCursorPosition([4, 10])).toMatchObject({
      type: 'text',
      literal: '2'
    });

    expect(doc.findNodeAtCursorPosition([5, 1])).toBeNull();
  });
  ...
  it('returns null if matched node does not exist', () => {
    ...
    // position in between two node (blank line)
    expect(doc.findNodeAtCursorPosition([2, 1])).toBeNull();

    // position out of document range
    expect(doc.findNodeAtCursorPosition([4, 1])).toBeNull();
  });
});
