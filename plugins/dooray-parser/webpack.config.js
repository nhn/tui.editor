...
function getOutputConfig(isProduction, isCDN, minify) {
  ...
  const defaultConfig = {
    library: {
      name: ['toastui', 'Editor', 'plugin', 'doorayParser'],
      export: 'default',
      type: 'umd',
    },
    environment: {
...
function getExternalsConfig() {
  return [
    {
      '@toast-ui/doorayParser': {
        commonjs: '@toast-ui/doorayParser',
        commonjs2: '@toast-ui/doorayParser',
        amd: '@toast-ui/doorayParser',
        root: ['toastui', 'doorayParser'],
      },
    },
  ];
}
