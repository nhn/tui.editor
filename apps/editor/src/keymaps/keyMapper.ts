...
class KeyMapper {
  ...
  convert(event: KeyboardEvent) {
    ...
    const keyChar = this.getKeyCodeChar(event.keyCode);

    if (keyChar) {
      keyMap.push(keyChar);
    }

    return keyMap.join(this.splitter);
  }
   ...
   * @private
   */
  private getKeyCodeChar(keyCode: number) {
