...
function removeMark(nodes: MdNode[], lineTexts: string[], newTr: Transaction) {
  ...
  const startPos: MdPos = [start[0], start[1] - 1];
  const endPos: MdPos = [end[0], end[1]];
...
function addMark(node: MdNode, lineTexts: string[], newTr: Transaction, schema: Schema) {
  const startPos: MdPos = [getMdStartLine(node), getMdStartCh(node) - 1];
  const endPos: MdPos = [getMdEndLine(node), getMdEndCh(node)];
