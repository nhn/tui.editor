...
class DefaultUI {
  ...
  _init({ el: container, toolbarItems, hideModeSwitch }) {
    ...
    this._initToolbar(this._editor.eventEmitter, toolbarItems);
    this._initModeSwitch(this._editor.eventEmitter, hideModeSwitch);
  ...
  _initEvent() {
    this._editor.eventEmitter.listen('hide', this.hide.bind(this));
    this._editor.eventEmitter.listen('show', this.show.bind(this));
    this._editor.eventEmitter.listen('changeMode', this._markdownTabControl.bind(this));
    this._editor.eventEmitter.listen('changePreviewStyle', this._markdownTabControl.bind(this));
  }

  _initToolbar(eventEmitter, toolbarItems) {
    const toolbar = new DefaultToolbar(eventEmitter, toolbarItems);

    this._toolbar = toolbar;
    this.el.querySelector(`.${CLASS_TOOLBAR}`).appendChild(toolbar.el);
  }

  _initModeSwitch(eventEmitter, hideModeSwitch) {
    ...
    const modeSwitch = new ModeSwitch(modeSwitchTabBar, editType, eventEmitter);
  ...
  _initMarkdownTab() {
    ...
    this._markdownTab.on('itemClick', itemText => {
      if (itemText === i18n.get('Preview')) {
        editor.eventEmitter.emit('previewNeedsRefresh');
        editor.eventEmitter.emit('changePreviewTabPreview');
        editor.eventEmitter.emit('closeAllPopup');
      } else {
        editor.getCodeMirror().focus();
        editor.eventEmitter.emit('changePreviewTabWrite');
      }
    });
  }
  ...
  _initPopupAddImage() {
    this._popups.push(
      new PopupAddImage({
        target: this.el,
        eventEmitter: this._editor.eventEmitter
      })
    );
  }
  ...
  _initPopupTableUtils() {
    this._editor.eventEmitter.listen('contextmenu', ev => {
      if (domUtils.parents(ev.data.target, '[contenteditable=true] table').length > 0) {
        ev.data.preventDefault();
        this._editor.eventEmitter.emit('openPopupTableUtils', ev.data);
      }
    });
