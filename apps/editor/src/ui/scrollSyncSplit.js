...
class ScrollSyncSplit {
  ...
  _setContentElement(element, side) {
    ...
    if (contentElement) {
      this._eventEmitter.removeEventHandler(EVENT_REQUIRE_SCROLL_INTO_VIEW);
    ...
    this._eventEmitter.listen(EVENT_REQUIRE_SCROLL_INTO_VIEW, ev =>
      this._requireScrollIntoView(ev)
    );
    this._eventEmitter.listen('requireScrollSync', () => this.sync());
