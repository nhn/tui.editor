...
class PopupDropdownToolbar extends LayerPopup {
  ...
  _initInstance(options) {
    ...
    const { button, eventEmitter } = options;

    this._button = button;
    this._eventEmitter = eventEmitter;
    this._toolbar = new Toolbar(eventEmitter);
  }
  ...
  _initEditorEvent() {
    ...
    this._eventEmitter.listen('focus', () => this.hide());
    this._eventEmitter.listen('closeAllPopup', () => this.hide());
    this._eventEmitter.listen(PopupDropdownToolbar.OPEN_EVENT, () => {
      const isShown = this.isShow();

      this._eventEmitter.emit('closeAllPopup');
      if (!isShown) {
        this.show();
      }
