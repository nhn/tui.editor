...
class Toolbar extends UIController {
  ...
  constructor(eventEmitter, items = []) {
    super({
      tagName: 'div',
   ...
   * @override
   */
  _initEvent(eventEmitter) {
    eventEmitter.listen('stateChange', ev => {
    ...
    eventEmitter.listen('changePreviewTabPreview', () => this.disableAllButton());
    eventEmitter.listen('changePreviewTabWrite', () => this.enableAllButton());
    eventEmitter.listen('changeMode', () => this.enableAllButton());
  }
  ...
  insertItem(index, item) {
    ...
    item.onCommandHandler = commandName => this._eventEmitter.emit('command', commandName);
    item.onEventHandler = eventName => this._eventEmitter.emit(eventName);
