...
class WysiwygEditor {
  ...
  _wrapDefaultBlockToOrphanTexts() {
    const textNodes = domUtils.getAllTextNode(this.getBody());

    textNodes.forEach(node => {
      if (node.nextSibling && node.nextSibling.tagName === 'BR') {
        domUtils.removeNode(node.nextSibling);
      }
  ...
  _joinSplitedTextNodes() {
    ...
    domUtils.removeNode(nodesToRemove);
  }
   ...
   * @returns {JQuery} jquery body
   * @todo change get$Body to this function
   */
  getBody() {
    return this.getEditor().getBody();
  }
  ...
  isEditorValid() {
    return this.getEditor() && domUtils.contains(document.body, this.editorContainerEl);
  }
