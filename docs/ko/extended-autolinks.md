...
[자동 링크](https://spec.commonmark.org/0.29/#autolinks)는 아래와 같은 CommonMark 스펙이다. (자동 링크의 세부 사양을 알고 싶으면 위 링크의 예를 참조바란다.)
> 자동 링크는 `<`, `>` 사이에 위치한 절대 경로 URI 또는 이메일 주소다. URL 또는 이메일 주소를 링크 레이블로 하여 구문 분석된다.
