...
```js
import 'tui-editor/dist/i18n/${fileName}';
```
...
You must register the language by including the language file you want to use. The `${fileName}` is corresponding to the 'I18N File' column in [Supported Languages](#supported-languages).
...
import 'tui-editor/dist/i18n/ko-kr'
